...
function uploadImgRules(file, LtSize = 2) {
  const _LtSize =
    getTranslateLink(
      globalVue.handleGlobalI18n('store', 'mgt_operation_image_size_notification_label')
    ) || LtSize
  const isIMG = file.type.slice(0, 5) === 'image'
  const isLtSize = file.size / 1024 / 1024 < _LtSize
  if (!isIMG) {
    globalVue.$message.error(
      globalVue.handleGlobalI18n('store', 'mgt_operation_image_format_notification_label')
    )
  }
  if (!isLtSize) {
    globalVue.$message.error(
      globalVue
        .handleGlobalI18n('store', 'mgt_operation_image_size_notification_label')
...
function genderFilter(gender) {
  const genderArr = [
    { value: 0, label: globalVue.handleGlobalI18n('store', 'mgt_empl_add_gender_unspecified_label') },
    { value: 1, label: globalVue.handleGlobalI18n('store', 'mgt_empl_add_gender_male_label') },
    { value: 2, label: globalVue.handleGlobalI18n('store', 'mgt_empl_add_gender_female_label') }
  ]
