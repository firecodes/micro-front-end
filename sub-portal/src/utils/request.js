...
const service = axios.create({
  ...
  baseURL: "https://fctest.accuat.com/fides/backend/v1",
  timeout: 0
});
...
service.interceptors.request.use(
  config => {
    ...
    if (getToken()) {
      ...
      config.url.indexOf(user.sendEmail) > -1 ||
      config.url.indexOf(user.changeForgot) > -1
    ) {
      config.headers["access_token"] = sessionStorage.getItem(
        "temporary_access_token"
      );
    } else if (
      config.url !== login &&
      config.url.indexOf(user.temporaryAccessToken) === -1
    ) {
      this.$router.push({
        path: "/login"
      });
    ...
    if (
      config.data &&
      config.url !== uploadImgUrl &&
      config.url !== uploadFileUrl &&
  ...
  response => {
    ...
    if (res.code !== RETURNCODE || res.code === "50114") {
      ...
      return {
        ...
        return_result: res.hasOwnProperty("result") ? res.result : null
      };
    }
  },
  error => {
