...
-->
<template>
  <div class="tier_downgrade_26" ref="tier_downgrade_26">
    <ed-table>
        ...
        <el-table-column :key="item.member_tier_id" :label="$t(`reporting.${item.member_tier_name}`)" :prop="item.prop" align="center" min-width="130" v-for="item in columns_list_data">
          <template slot-scope="scope">
            ...
            <template v-else-if="item.member_tier_id  !== '-999999'">
              {{
              function () {
              var value = (scope.row)[item.prop + ' Rate']
              if(value === '-') {
              return value
              } else if(value) {
...
<script>
  import { tierDowngrade } from '@/api/report'
  import { tier_downgrade_25 } from '@/data/reporting'
  export default {
    data() {
      return {
        table_data: [],
        ...
        if (source === 'dw') {
          tierDowngrade(obj).then(res => {
            ...
            if (result.columns_list && result.columns_list.length > 0) {
              c_data = [{ member_tier_name: 'fc_reporting_oneclick_downgrade_rate_table_upgrade_lable', member_tier_id: '-999999', prop: 'current_tier' }].concat(result.columns_list)
          ...
          var _this = this
          setTimeout(() => {
            _this.columns_list_data = tier_downgrade_25.columns_list
            _this.table_data = tier_downgrade_25.json_data_array
      ...
      export() {
        import('@/vendor/Export2Excel').then(excel => {
          ...
          for (var item of this.columns_list_data) {
            ...
            if (item.prop === 'current_tier') {
              filterVal.push(item.prop)
            } else {
              filterVal.push(item.prop + ' Rate')
            }
          }
          ...
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: `${this.$t('reporting.fc_reporting_oneclick_search_type_menu19_lable')}-${Date.parse(new Date())}`,
            autoWidth: 250
          })
        })
      },
