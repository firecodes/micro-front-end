...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        if (this.target) {
          const list = this.target.target_profiles && this.target.target_profiles[1].criteria_fields ? this.target.target_profiles[1].criteria_fields : []
          if (list.length <= 0) {
            return true
          }
      ...
      closeDialog() {
        const obj = this.$refs['product-list'].change()
        this.dialog = false
