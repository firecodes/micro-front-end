<!--Past-->
<template>
  <section class="spot_campaign__past">
    <ed-header-title :title="$t('campaign.fc_campaign_spot_past_section_label')" />
    <div class="spot-campaign__table">
      <ed-table>
        <el-table :data="table_data" align="center" style="width: 100%" v-loading="table_loading">
          <!--Campaign name-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_past_table_name_label')" align="center"  prop="campaign_name">
            <template slot-scope="scope">
              <ed-clamp :max-lines="1" autoresize>{{scope.row.campaign_name || table_placeholder}}</ed-clamp>
            </template>
          </el-table-column>
          <!--Reward type-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_past_table_type_label')" align="center"  prop="campaign_type_code">
            <template slot-scope="scope">
              {{scope.row.campaign_type_code || table_placeholder}}
            </template>
          </el-table-column>
          <!--Estimated audience-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_past_table_audience_label')" align="center" prop="estimated_audience">
            <template slot-scope="scope">
          ...
          <!--Send date-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_past_table_date_label')" align="center" prop="send_date">
            <template slot-scope="scope">
              {{scope.row.send_date ? globalData.formatDate( formatTimeStamp(scope.row.send_date).substr(0, 10)) : table_placeholder}}
            </template>
          </el-table-column>
          <!--Operation-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_past_table_operation_label')" align="center" prop="address" width="195">
            <template slot-scope="scope">
