<template>
  <div class="select-sku-dialog">
    ...
    <ed-dialog>
      <!--select a sku-->
      <el-dialog :before-close="handleClose" :title="$t('campaign.fc_campaign_spot_create_select_sku_title_label')" :visible.sync="dialog_visible" @close="close" @open="init" custom-class="select-sku-custom-class" width="700px">
        <div class="search">
    ...
    methods: {
      close() {
        ...
        if (this.value_list.length <= 0) {
          ...
          this.$refs.select_sku_table.clearSelection()
        }
      },
      init() {
        setTimeout(() => {
      ...
      getList(obj = {}) {
        ...
        queryProductList(Object.assign({}, this.list_query, obj, { product_query: this.form.product_query.trim() })).then(res => {
