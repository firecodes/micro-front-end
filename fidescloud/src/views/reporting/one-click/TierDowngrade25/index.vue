...
-->
<template>
  <div class="tier_downgrade_25" ref="tier_downgrade_25">
    <ed-table>
        ...
        if (source === 'dw') {
          tierDowngrade(obj).then(res => {
            ...
            if (result.columns_list && result.columns_list.length > 0) {
              c_data = [{ member_tier_name: 'fc_reporting_oneclick_tier_downgrade_table_downgrade_lable', member_tier_id: '-999999', prop: 'current_tier' }].concat(result.columns_list)
      ...
      export() {
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: `${this.$t('reporting.fc_reporting_oneclick_search_type_menu18_lable')}-${Date.parse(new Date())}`,
            autoWidth: 250
          })
        })
      },
