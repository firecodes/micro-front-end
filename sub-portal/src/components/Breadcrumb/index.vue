    ...
    <transition-group name="breadcrumb">
      ...
      >
        <span
          ...
          class="de-redirect"
          :class="index == levelList.length - 1 ? 'no-redirect' : ''"
        >
    ...
    handleRouter(item) {
      ...
      if (item.hasOwnProperty("parent")) {
        if (
          ...
          item.parent.meta.hasOwnProperty("title")
        ) {
      ...
      if (second !== undefined) {
        if (second.meta.hasOwnProperty("parents")) {
          ...
          if (second_params_parent_length > 0) {
            if (
              second_params_parent[second_params_parent_length - 1].path !==
              second_parent
            ) {
              matched.splice(1, 0, {
                path: `/${second_parent}/${second_params_parent[0].path}`,
                meta: {
                  title: second_params_parent[0].name,
                  transaction_key: second_params_parent[0].transaction_key
                }
              });
              for (let i = 1; i < second_params_parent_length; i++) {
                matched.splice(i + 1, 0, {
                  path: `/${second_params_parent[i - 1].path}/${
                    second_params_parent[i].path
                  }`,
                  meta: {
                    title: second_params_parent[i].name,
                    transaction_key: second_params_parent[0].transaction_key
                  }
                });
              }
