  ...
  <div class="r-EmployeeWrapper">
    <el-form v-loading="loading" :model="addForm" :validate-on-rule-change	="false" :rules="addFormRules" status-icon label-width="122px" style="width: 555px;" class="addForm" ref="addForm">
      <el-form-item :label="handleGlobalI18n('employee', 'mgt_empl_add_last_label')" prop="last_name">
          <el-input disabled type="text" v-model="addForm.last_name" maxlength="25" auto-complete="off" clearable :placeholder="handleGlobalI18n('employee', 'mgt_empl_add_last_input_label')"></el-input>
      </el-form-item>
      <el-form-item :label="handleGlobalI18n('employee', 'mgt_empl_add_first_label')" prop="first_name">
        <el-input disabled type="text" v-model="addForm.first_name" maxlength="25" clearable :placeholder="handleGlobalI18n('employee', 'mgt_empl_add_first_input_label')"></el-input>
      </el-form-item>
      <el-form-item :label="handleGlobalI18n('employee', 'mgt_empl_add_gender_label')" prop="gender" clearable>
        <el-select disabled v-model="addForm.gender" value-key="id" @change="selectGender" style="width: 250px;" :placeholder="handleGlobalI18n('employee', 'mgt_empl_add_gender_input_label')">
      ...
      <el-form-item :label="handleGlobalI18n('employee', 'mgt_empl_add_position_label')" prop="position">
        <el-select disabled value-key="id" @change="clickTypeList" v-model="addForm.position" clearable :placeholder="handleGlobalI18n('employee', 'mgt_empl_add_position_list_label')" style="width: 250px; margin-right: 10px;">
          <el-option v-for="(item) in typeList" :key="item.id" :value="item.id" :label="item.name"></el-option>
        </el-select>
        <el-button disabled type="primary" class="add-type" @click="addPositionDialog = true">{{ handleGlobalI18n('employee', 'mgt_empl_add_position_add_label') }}</el-button>
      </el-form-item>
      <!-- store -->
      <el-form-item :label="handleGlobalI18n('employee', 'mgt_empl_add_store_label')" prop="position">
        <el-select disabled value-key="id" v-model="addForm.store" clearable :placeholder="handleGlobalI18n('employee', 'mgt_empl_add_store_list_label')" style="width: 250px; margin-right: 10px;">
          <el-option v-for="(item) in storeLists" :key="item.id" :value="item.id" :label="item.store_name"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="handleGlobalI18n('employee', 'mgt_empl_add_mobile_label')">
        <div style="display: flex">
          <el-form-item prop="phonePrefix" style="width: 130px;">
            <el-select disabled v-model="addForm.phonePrefix" >
          ...
          <el-form-item prop="mobile" style="width: 300px;">
            <el-input disabled type="text" v-model="addForm.mobile" maxlength="11" :placeholder="handleGlobalI18n('employee', 'mgt_empl_add_mobile_input_label')"></el-input>
          </el-form-item>
        </div>
      </el-form-item>
      <el-form-item :label="handleGlobalI18n('employee', 'mgt_empl_add_email_label')" prop="email">
        <el-input disabled type="text" v-model="addForm.email" maxlength="32" clearable :placeholder="handleGlobalI18n('employee', 'mgt_empl_add_email_input_label')"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button @click="$router.go(-1)">{{ $t('loyalty.fc_operation_btn_back_label') }}</el-button>
        <el-button type="primary" @click="submitForm('addForm')" :loading="btnLoading">{{ handleGlobalI18n('employee', 'mgt_operation_btn_edit_label') }}</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-dialog
      ...
      <el-table :data="addPositionList" border>
        <el-table-column :label="handleGlobalI18n('employee', 'mgt_empl_position_table_position_label')" prop="name" min-width="300">
        ...
        <el-table-column :label="handleGlobalI18n('employee', 'mgt_empl_position_table_operation_label')"  width="210px">
          <template slot-scope="scope">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="addType">{{ handleGlobalI18n('employee', 'mgt_empl_position_window_add_label') }}</el-button>
        <el-button type="primary" @click="addPositionDialog = false">{{ handleGlobalI18n('employee', 'mgt_empl_position_window_close_label') }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
// updateEmployee insertEmployee
import { employeeDetail, queryEmployeeRole, setEmployeeRoleActive, updateEmployeeRole, insertEmployeeRole } from '@/api/employ'
...
export default {
  ...
  methods: {
    ...
    getStoreList() {
      return storeList({
        ...
        if (res.return_result) {
          this.storeLists = res.return_result.results
        }
    ...
    submitForm() {
      this.$router.push(`/configuration/employee/list/edit/${this.$route.params.id}`)
    },
    // add position
