...
export const addSubCategory = {
  ...
  methods: {
    async getSubCategorylists() {
      this.loading = true;
      await getCategoryList(this.listQuery)
        .then(res => {
    ...
    cancelAddSubCategory(index, row) {
      ...
      if (row.id) {
        this.$set(
          ...
          idx,
          Object.assign({}, result, { type: "none", name: result.oriName })
        );
      }
      if (!row.id) {
    ...
    editSubCategory(index, row) {
      ...
      this.$set(
        this.subCategoryList,
        idx,
        Object.assign({}, result, { type: "addOrEdit", oriName: result.name })
      );
    },
    saveSubCategory(row, index) {
      ...
      if (code) {
        ...
        this.updateSub({
          id,
          name: name ? name.trim() : "",
          category_parent_id: this.listQuery.category_parent_id
        });
      } else {
        // add
        this.addSub({
    ...
    deleteSubCategory(index, id) {
      this.deleteItem(
        ...
        this.$t(
          "configuration.fc_product_sub_category_delete_popup_content_label"
        )
      )
        .then(res => {
          this.loading = true;
