<template>
  <div class="content" ref="dynamic_content">
    <el-form>
      <ul>
        <li v-for="(item, index) in list" :key="index">
          <div class="from"><el-input maxlength="32" v-model="item.from"></el-input></div>
          <div class="to"><el-input maxlength="64" v-model="item.to"></el-input></div>
    ...
    <div class="add" @click="addLi" :class="{'margin-top-8' : list.length <= 0}">
      <el-button class="new-button-not-primary content-add-button">{{$t('loyalty.fc_add_gift_content_btn_add_label')}}</el-button>
    </div>
  </div>
</template>
<script>
