...
export default {
  install(Vue, options) {
    ...
    Vue.directive('selectLoadmore', {
      bind(el, binding) {
        const SELECTWRAP_DOM = el.querySelector('.el-select-dropdown__wrap') || el.querySelector('.el-scrollbar__wrap')
        SELECTWRAP_DOM.addEventListener('scroll', () => {
          let sign = 40
          const CONDITION = ((SELECTWRAP_DOM.scrollHeight - SELECTWRAP_DOM.scrollTop === SELECTWRAP_DOM.clientHeight) && SELECTWRAP_DOM.scrollTop > sign)
