      ...
      <el-form-item :label="$t('campaign.fc_campaign_spot_create_what_conditions_label')">
        <div class="list">
          ...
          <div class="spot_campaign_what_multiple_point__transaction_value">
            <el-checkbox v-model="form.transaction" />
            <!--Transaction value -->
            ...
            <el-form-item class="border-bottom-none" prop="purchase_amount_restriction_from">
              <el-input class="margin-27-10" oninput="value=value.replace(/[^\d.]/g,'')" v-model.number="form.purchase_amount_restriction_from" />
            </el-form-item>
            <label>{{globalData.value.currency}}</label>
            <span>—</span>
            <el-form-item class="border-bottom-none" prop="purchase_amount_restriction_to">
              <el-input class="margin-10" oninput="value=value.replace(/[^\d.]/g,'')" v-model.number="form.purchase_amount_restriction_to"  />
            </el-form-item>
            <label>{{globalData.value.currency}}</label>
          </div>
          <!--SKU-->
          <div class="spot_campaign_what_multiple_point__select_sku">
            <el-checkbox v-model="form.select_sku" />
            ...
            <div style="width: 480px">
              ...
              <pagination-container :page = "list_query.page_no" :page_size = 'list_query.page_size' :total = 'list_query.total' @handleCurrentChange = "handleCurrentChange" layout="total, prev, pager, next, jumper" v-if="list_query.total > 0" />
            </div>
          </div>
          ...
          <div class="spot_campaign_what_multiple_point__participant_limit">
            ...
            <el-input-number :max="9999999" :min="1" :precision="0" v-model="form.participant_limited_times" />
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          purchase_amount_restriction_from: [{ validator: this.handlePurchaseAmountRestrictionFrom, trigger: 'blur' }],
          purchase_amount_restriction_to: [{ validator: this.handlePurchaseAmountRestrictionTo, trigger: 'blur' }]
        },
        table_data: [],
        show_table_data: [],
        table_loading: false,
    ...
    methods: {
      ...
      init() {
        if (this.reward) {
          this.form = {
            ...
            participant_limited_times: this.reward.participant_limited_times || 1,
          ...
          if (this.form.select_sku) {
            ...
            if (this.reward.purchase_product_limits.length) {
              for (const item of this.reward.purchase_product_limits) {
      ...
      handlePurchaseAmountRestrictionTo(rule, value, callback) {
        ...
        this.$refs['form_m_p'].validateField('purchase_amount_restriction_from')
        callback()
      },
      selectSkuData(arr) {
        this.table_data = arr
      ...
      saveData() {
        ...
        return new Promise((resolve, reject) => {
          this.$refs['form_m_p'].validate((valid) => {
            if (valid) {
              resolve({
                participant_limited_times: this.form.participant_limit ? this.form.participant_limited_times : null,
