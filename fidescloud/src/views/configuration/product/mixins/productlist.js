...
export const productListMixins = {
  ...
  methods: {
    getList() {
      return getProductList(this.listQuery).then(res => {
    ...
    async handleAdvacedSearch(query, flag) {
      if (!flag) {
        ...
        this.listQuery = { page_no: 1, page_size: 10, condition: '', asc: false }
      }
      ...
      await advancedSearch({ ...query, ...this.listQueryAdvanced }).then(res => {
        if (res.return_result) {
          const result = res.return_result
          this.total_count = result.total_count
          this.list = result.results
    ...
    handleDelete(id) {
      this.deleteItem(this.$t('loyalty.fc_operation_delete_confirm_title_label'), this.$t('configuration.fc_product_delete_popup_content_label')).then(res => {
        this.loadingTable = true
        return deleteProduct({
          id
        }).then(async res => {
          if (res.return_result) {
            this.listQuery = { page_no: 1, page_size: this.listQuery.page_size, condition: '', asc: false }
    ...
    handleExport() {
      ...
      const { code = null, name = null, category = null, brand = null } = this.advacnedInfo
      const obj = {
        'asc': false,
        'coding': 'utf-8',
        'code': code,
        ...
        'header': {
          ...
          'retail_market_price': this.$t('configuration.fc_product_table_market_price_label'),
          'purchased_price': this.$t('configuration.fc_product_table_purchased_price_label')
        }
      }
      return productExport({ ...obj }).then(res => {
