    ...
    <div v-loading="list_loading">
      <ed-table class="box-shadow-none">
        <el-checkbox-group v-model="check_arr">
          ...
          <!--Channel-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_create_how_edm_table_channel_label')" align="center">
            <template slot-scope="scope">
              EDM
            </template>
          </el-table-column>
          <!--Subject-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_create_how_edm_table_subject_label')" align="center">
            <template slot-scope="scope">
              <ed-clamp :max-lines="1" autoresize>{{scope.row.subject || table_placeholder}}</ed-clamp>
            </template>
          </el-table-column>
          <!--Content-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_create_how_edm_table_content_label')" align="center">
            <template slot-scope="scope">
              <ed-clamp :max-lines="1" autoresize>{{scope.row.content || table_placeholder}}</ed-clamp>
            </template>
          </el-table-column>
          <!--Creation date-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_create_how_edm_table_create_label')" align="center">
            <template slot-scope="scope">
              {{scope.row.create_date ? globalData.formatDate( formatTimeStamp(scope.row.create_date).substr(0, 10)) : table_placeholder}}
            </template>
          </el-table-column>
          <!--Last updated date-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_create_how_edm_table_update_label')" align="center" prop="effective_from">
            <template slot-scope="scope">
              {{scope.row.modify_date ? globalData.formatDate( formatTimeStamp(scope.row.modify_date).substr(0, 10)) : table_placeholder}}
            </template>
          </el-table-column>
          <!--Operation-->
          <el-table-column :label="$t('campaign.fc_campaign_spot_create_how_edm_table_operation_label')" align="center" width="200px">
            <template slot-scope="scope">
              <div class="operation">
                <!--Edit-->
                <label @click="handleEditTemplate(scope.row)">{{$t('campaign.fc_campaign_spot_create_how_edm_btn_edit_label')}}</label>
                <!--Delete-->
                <label @click="handleDelete(scope.row.id)">{{$t('campaign.fc_campaign_spot_create_how_edm_btn_delete_label')}}</label>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        if (Object.keys(this.select_obj).length > 0 && this.table_data.length) {
