<template>
  <div class="lifecycle-campaign-how-item" ref="how">
    <el-form label-width="150px" >
      <p class="p">{{$t('campaign.fc_campaign_lifecycle_edit_how_title_label')}}</p>
      <!--选择模式-->
      <div class="select-mode">
        <el-form-item :label="$t('campaign.fc_campaign_lifecycle_edit_how_channel_label')">
      ...
      <div class="sms-mode" v-if="form.kind === 'SMS'">
        <div class="left">
          ...
          <div class="preview">
            <el-button>{{$t('campaign.fc_campaign_lifecycle_edit_how_btn_preview_label')}}</el-button>
          </div>
        </div>
        <div class="right">
                ...
                <li v-for="(item, index) in options[0].template" v-if="item!== null" :class="{'item_check': sms_id === index }"  @click="sms_id = index" >
                  ...
                  <div class="bottom-button">
                    <div class="edit" @click="dialog_state = true; textarea2 = item.content; edit_state = true; edit_id = item.id">
                      ...
                      <p>{{$t('campaign.fc_campaign_lifecycle_edit_how_btn_edit_label')}}</p>
                    </div>
                  </div>
                </li>
              </ul>
      ...
      <el-dialog :title="handleGlobalI18n('campaign', 'edit')" :visible.sync="dialog_state" width="1000px">
        ...
        <div style="margin-top:14px; text-align: center" class="dialog-sms-button">
          <el-button @click="dialog_state=false" class="cancel">{{handleGlobalI18n('campaign', 'cancel')}}</el-button>
          <el-button type="primary"  @click="addOrUpdateSmsTemplate(1)" :loading="sms_loading_dialog">{{handleGlobalI18n('campaign', 'confirm')}}</el-button>
        </div>
      </el-dialog>
      <!--WeChat  编辑或者添加-->
      <el-dialog :title="handleGlobalI18n('campaign', 'edit')" :visible.sync="dialog_state_wechat" v-if="options[2] !== undefined" width="1000px">
        <div class="ld">
          <div class="valiable">
            <label>{{handleGlobalI18n('campaign', 'variable')}}:</label>
            <div style="overflow-y: hidden;min-height: 300px;width: 250px">
        ...
        <div style="margin-top:14px; text-align: center" class="dialog-sms-button">
          <el-button @click="dialog_state_wechat=false" class="cancel">{{handleGlobalI18n('campaign', 'cancel')}}</el-button>
          <el-button type="primary"  @click="addOrUpdateWechatTemplate(3)" :loading="wechat_loading_dialog">{{handleGlobalI18n('campaign', 'confirm')}}</el-button>
        </div>
      </el-dialog>
      <!--EDM 编辑或者添加-->
      <dialog-edm
    ...
    methods: {
      ...
      init() {
        return new Promise(resolve => {
          ...
          const communication_definition_list = info.communication_definition_list[0]
          this.form = { ...communication_definition_list }
          if (Object.keys(this.form).length > 0) {
            if (this.form.kind === 'SMS') {
              this.options[0].template.forEach((item, index) => {
                if (parseInt(item.id) === parseInt(this.form.communication_template_id)) {
                  this.sms_id = index
                }
              })
            } else if (this.form.kind === 'EDM') {
              this.options[1].template.forEach((item, index) => {
                if (parseInt(item.id) === parseInt(this.form.communication_template_id)) {
                  this.edm_id = index
                }
              })
            } else {
              this.options[2].template.forEach((item, index) => {
                if (parseInt(item.id) === parseInt(this.form.communication_template_id)) {
    ...
    watch: {
      ...
      form_data: {
        async handler(val) {
          await this.init()
          await this.getCommunicationChannelList()
          await this.getTemplateList(this.form.kind)
          await this.getReservedFieldList()
        }
      },
      nonius_id: {
