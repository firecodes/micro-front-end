...
export const addProduct = {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        code: [{ required: true, validator: this.handleBlurCode, trigger: 'blur' }],
        unit: [{ validator: validateUnit, trigger: ['change', 'blur'] }],
        marketPrice: [{ validator: validateMarketPrice, trigger: ['change', 'blur'] }],
  ...
  methods: {
    async getProductDetail(id) {
      this.loading = true
      await productDetail({ id }).then(async(res) => {
        if (res.return_result) {
          const { code, name, brand, brand_name, category, category_name, parent_category, parent_category_name, unit, retail_market_price, purchased_price, images, desc } = res.return_result
          this.ruleForm.code = code
          this.ruleForm.name = name
          this.ruleForm.brand = brand_name ? +brand === 0 ? '' : brand : ''
          this.ruleForm.category = parent_category_name ? +parent_category === 0 ? '' : parent_category : ''
          await this.getAllSubCategory(this.ruleForm.category)
          this.ruleForm.subCategory = category_name ? +category === 0 ? '' : category : ''
          if (unit) {
      ...
      if (this.ruleForm.desc) {
        this.ruleForm.detailInformation = await this.handleHtml2str(this.ruleForm.desc)
      } else {
    ...
    // 获取brand
    getAllBrand() {
      return getBrandList({ page_no: 1, page_size: 1000 }).then(res => {
        if (res.return_result) {
    ...
    // 获取category
    getParentCategory() {
      return getCategoryList(this.parentCategoty).then(res => {
        if (res.return_result) {
    ...
    beforeAvatarUpload(file) {
      ...
      if (!isGoOnUpload) {
        file.times = 'error'
        this.$message.error('Uploading pictures with the same name (i.e. the same name and format) is not allowed')
        return false
      }
    ...
    changeUnitPrice(type) {
      this.$nextTick(() => {
        if (this.ruleForm[type]) {
          ...
          this.ruleForm[type] = String(this.ruleForm[type]).replace(/[^\d.]/g, '')
    ...
    replaceDot(val, type) {
      let flag = true
      val && String(val).replace(/\./ig, (a, b, c) => {
        if (!flag) { // 不是第一个
          this.ruleForm[type] = this.ruleForm[type].substring(0, this.ruleForm[type].lastIndexOf('.'))
          return ''
        } else {
          flag = false
          return '.'
    ...
    handleBlurCode(rule, value, callback) {
      ...
      return productCheckCode({ code: value.trim() }).then(res => {
        if (res.return_result) {
          if (res.return_result.success) { // 此code已经存在
      ...
      this.$refs['ruleForm'].validate(async valid => {
        if (valid) {
          ...
          if (this.ruleForm.detailInformation) {
            await this.handleStr2html(this.ruleForm.detailInformation, true).then(url => {
          ...
          const { brand, category, subCategory, code, name, unit, marketPrice: retail_market_price, price: purchased_price, detailUrl, checked, unitselect } = this.ruleForm
          ...
          const paramsData = {
            ...
            brand: brand === '' ? 0 : brand,
            // 现在的sub category
            category: category === '' ? 0 : subCategory === '' ? 0 : subCategory,
            // 之前的category
