...
export function formatMonty(m) {
  ...
  if (m > 0 && m < 1000) {
    ...
    return (m / 100000000).toFixed(2) + '<span style="font-size:18px;"> B</span>'
  }
}
...
export function date(obj, spacer = '-') {
  ...
  return y + spacer + m.substring(m.length - 2, m.length) + spacer + d.substring(d.length - 2, d.length)
}
...
var getUrlHost = function() {
  ...
  return host !== 'localhost:9528' ? `${protocol}//${host}` : process.env.BASE_URL
